...
** DOIN [5/38] Finale Planne whatever
...
**** WAIT [0/3] Normalization
- [ ] see if the log-mel-spec values are in good range
- [ ] Choose if CMN, or overall/frequency axis dynamic range normalization
...
- [ ] How many total steps are there in the training data per sweep?
  + [ ] on myrmidon
  + [ ] on unumpu
**** WAIT [0/5] Number of epochs / passes over the dataset - =steps_per_chunk epochs per pass= * {=5=, =11=, =21= =passes=}
...
**** WAIT [0/4] The Neural =Network= - {~c3~}
...
#+END_SRC
**** WAIT [2/3] Features to use - {=fbanks_64=}
