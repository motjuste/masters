...
content, and the presence of hesitations, self-corrections, and other
disfluencies (including double talk situations), make conversations in meetings
or other informal settings much more challenging than planned and/or read speech
(e.g. TV or radio broadcast recordings). Due to the detrimental impact of
overlapping speech on their performance \cite{cetin_speaker_2006,
morgan_meeting_2001, shriberg_spontaneous_2005}, such systems often remove them
in a pre-processing step, or mitigate for such situations with the help of extra
information. In an automatic speaker identification / verification system, for
example, individual speaker models must be learned on non-overlapping examples
to ensure purity, and the system should be robust against the presence of
interfering speakers in real world application, or at least refuse to assign a
speaker by identifying presence of competing speakers.
...
party problem/. Humans listeners are able to focus their attention on a single
talker among a mixture of conversations and background noises, for instance,
during a loud crowded party. This perceptual /separation of sources/ is not very
...
overlapping speech in order to separate the contributing sources, whereas the
...
hinted in the previous section: even though they are frequent, double talk
situations are predominently of short duration, which result in heavy imbalance
between the examples for single-speaker and overlapping-speakers classes. The
conversations themselves are often recorded in less than ideal conditions (e.g.
varying degrees of interfering noise, etc.). Many studies have therefore chosen
to develop and evaluate their proposed approaches on artificially created
overlapping speech.
...
signal, and were then applied to power-spectrograms in the form of a binary mask
to obtain the Pyknograms. These were shown to have relatively smooth patterns
when the underlying speech was produced by a single speaker, but had
discontinuities in cases where there were multiple simultaneous speakers, which
were measured by calculating average distance between neighboring units in time
over at least a 2 second long segment. Shokouhi /et al./ compared their approach
with the performance of other popular ones that used speech kurtosis, Spectral
Flatness (Section [[Speaker Diarization]]), or Spectral Autocorrelation Peak-Valley
Ratio (SAPVR). The change in kurtosis (fourth order moment of a random variable)
of speech signal could be a consequence of increased signal complexity, and has
been used by several studies
...
peak-to-valley ratios in the speech signal's spectrum are used with the
...
other approaches under several base-speaker's to interfering-speaker's power
ratios. Nevertheless, they reported significant performance degradation when the
overlapping speech segments were less than 2 seconds long, which, setting aside
issues related to use of artificially overlapped speech (discussed in Section
[[Tackling Class Imbalance]]), does not enthuse one to apply this method on real conversations
where such overlaps are usually much smaller.

Earlier, Lewis /et al./ \cite{lewis_cochannel_2001} developed Pitch Prediction
...
estimated pitch peaks are more regular in single speaker case than in two
speaker case. This feature was shown to outperform the standard acoustic
features like Linear Predictive Coding (LPC) coefficients and Mel-Spectral
Frequency Coefficients (MFCCs) (Section [[Speaker Diarization]]). However, Wrigley
/et al./ \cite{wrigley_speech_2005} reported that MFCCs, PPF and SAPVR did not
give satisfactory results for detecting overlapping speech in real
conversations. Approaches that have been developed and evaluated on real
conversations are discussed next.
...
and retrieval, etc. For several conversation analysis workflows, an ideal
version of such system is almost perfect, especially when combined with a robust
...
for other speech technologies like Automatic Speech Recognition (ASR), speaker
...
from a speaker diarization step to concentrate on only the segments with speech,
...
3) **Clustering** the same speaker segments based on some acoustic similarity
   ...
   iterative adaptation of speaker models based on current clustering is done
...
\cite{otterson_efficient_2007}. Anguera /et al./ in fact call overlapping speech
the /'Achilles heel'/ of speaker diarization for meetings.

The most common approach to solve for this employs a separate model-based overlap detection
system whose predictions about the presence of overlapping speech can be used in
the pre-processing step to remove such segments and then in the labelling step
to signal the need to look for a second contributing speaker
...
detection system, either do the detection and re-segmentation of the diarization
system's output by adding to the derived speaker models all the possible
combinations of such models \cite{van_leeuwen_ami_2006}, or, integrate these
combinations in the speaker segmentation and clustering stage of the diarization
system itself \cite{lathoud_location_2003,lathoud_segmenting_2003}. These other
approaches, however, have not been shown to improve the overall diarization
performance in real conversations \cite{van_leeuwen_ami_2006}, or require
information about the total number of speakers to be known beforehand
...
Almost all dedicated overlap detection systems use GMM-HMM based framework for
...
based overlap detection system in \cite{geiger_detecting_2013}. In most cases
non-speech parts are removed beforehand by using a speech activity detector (or
by using ground-truth labels) so that the models are learned for single-speech
and overlapping-speech. Then the results of the speech activity detector (or
ground-truth labels) are integrated into the decoding phase to produce the final
segmentation of the audio for the three classes.

In all GMM-HMM based frameworks, a three-state Hidden Markov Model (HMM) is
trained for each of three classes while the state emission probabilities are
modelled by using a multivariate Gaussian Mixture Model (GMM) learned from
various acoustic features. The posteriors for the three classes are then decoded
to get the final classification using Viterbi Algorithm with an Overlap
...
precision, paired with 40% to 20% for recall, when evaluated on subsets of AMI
~[]~ and/or NIST RT ~[]~ corpora for different acoustic feature combinations.
And, in general, longer duration overlaps are more well detected than shorter
duration ones. Such steep tradeoff between precision and recall has lead to the
claim that overlapping speech detection in conversations is still an extremely
challenging problem.

Nevertheless, as is common in many classical investigations for developing a
pattern recorgnition system, various groups have explored the use of different acoustic
feature combinations in their studies for overlap detection. In general, using only Mel-Frequency
Cepstral Coefficients (MFCCs; Section [[Acoustic Features]]) has not been found to
be sufficient, the use of which for speaker diarization (which requires
invariance against what is being spoken) and also for speech recognition (which
requires invariance against who is speaking) has, at least, lead to a cognitive
dissonance \cite{anguera_speaker_2012}. Some of the milestone studies are
discussed below.
...
various acoustic features for training a GMM-HMM based system for detecting
...
entropy in the frame-wise speaker likelihood predictions produced by a
...
recording. SF are calculated as the ratio between the geometric and arithmetic
mean of spectral magnitudes over some number of temporal frames, and can have
relatively higher values in regions of overlap than single speaker regions.
Principal Component Analysis (PCA) and Artificial Neural Networks (ANN) were
investigated to transform the cross-correlation based features to reduce their
dimensionality and also to make them independent of the number of microphones
used in the recording. While spectral features performed well, they were
outperformed when they were combined with cross-correlation based features.
...
of speech, and discussed earlier in Section [[Double-Talk in Conversations]] can
trigger listener responses that can occur in the form of overlaps. However,
...
Relevance (mRMR) to individually score each candidate feature against the target
classes of single-speech v/s overlapping-speech, and then an iterative hill
climbing wrapper approach where the best scoring prosodic features were
...
Geiger /et al./ \cite{vipperla_speech_2012,geiger_speech_2012-1} investigated
the using Convolutive Non-negative Sparse Coding (CNSC) for overlap detection
...
respected to overlap detection. They particularly pointed out the difficulty in
detecting short-duration overlaps which are, nonetheless, the most common type
of overlaps (Section [[Double-Talk in Conversations]]).

Geiger /et al./ \cite{geiger_detecting_2013} later also investigated the use of
Long Short-Term Memory (LSTM) based Recurrent Neural Networks (RNNs) for
detecting overlaps. A single layer network with four memory block, each with 50
...
Charlet /et al./ \cite{charlet_impact_2013} focused on detecting overlapping
...
than one second long, reporting great precision and recall, because their
ultimate goal was to evaluate how can the performance of their diarization
system be improved with the exclusion of detected overlaps and later attribution of
a second speaker based on two nearest speakers in time. In that respect, they
report significant improvement in the diarization error rate.
