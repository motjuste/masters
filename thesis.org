...
** DOIN [0/38] Finale Planne whatever
...
- [ ] Use =librosa.features.melspectrogram=, and then =librosa.logamplitude=
  with ref = ~1.0~
  + see if it gives the values in a good range
...
- [ ] keep the mel-frequencies to be added to the dataset h5, probably
...
- [ ] look at links on MFCC, iVectors, prosody, pitch, CNSC, PLP, etc. for inspiration.
  + [ ] See if the argument that 'spectrogram' is the mother of all features
    still holds.
*** WAIT [0/18] The Classifier and Configurations
...
- [ ] How many total steps are there in the training data per sweep?
  + [ ] on myrmidon
  + [ ] on unumpu
**** WAIT [0/5] Number of epochs / passes over the dataset - =steps_per_chunk epochs per pass= * {=1=, =6=, =11=, =21= =passes=}
...
**** WAIT [0/3] The Neural =Network= - {~c3~}
...
#+END_SRC
**** WAIT [0/1] Features to use - {~fbanks_64~}
- choose one between ~{40, 64, 96}~, and stick to it.
...
**** WAIT [0/1] =Sub-sampling= class(-es) {=1T_0.25=, =None=}
...
***** TODO [0/0] How to subsample, the algo
...
**** WAIT [0/5] Choosing =label= for a sequence - {=center-frame=, =max-mid-±5-frames=}
...
- [ ] maybe be smarter about creating strided_views based on data_ctxt and label_ctxt
**** WAIT [0/2] Save model =checkpoint= on every keras epoch - {=per-keras-epoch=}
- [ ] decide on file name formatting.
  + should reflect the true epoch and sub-epoch number
  + [ ] Add a convenience function to =keras_utils= that accepts activity path
- Save checkpoints every keras epoch
**** WAIT [0/2] Save =Tensorboard= events {=per-keras-epoch=}
...
**** TODO [0/3] Part of =validation data= to use - {~('00003', '00086')~}
...
- [ ] *Find 1 or two calls after data-analysis over the extracted frames*
...
**** WAIT [0/9] Chatty =Confusions= in callback - {=init+per-keras-epoch=}
- [ ] Add to a new =keras_utils= file.
- [ ] requires path to export the h5 to.
- [ ] Make a confusion calculation on init, on 0th batch of 0th epoch
...
- [ ] After the init one, Only do per keras epoch, and we have 8*npasses any way.
  + [ ] how to calculate preds? =predict_generator=?
  + [ ] print out the precision and recall for all classes being trained on, with correct
    epoch and sub-epoch number.
  + [ ] Save the full confusion matrix as well.
- [ ] print full confusion after training ends.
- [ ] Make sure that there is no code being called that needs packages not on
  the GPU.
- [ ] Structure of the h5?
- [ ] use the same val_dp as one given to keras for it's validation
...
*** TODO [0/0] The Table of Experiments
**** Fixed
...
| features            | fbank ~64~                    |
| data_ctxt           | ±10                           |
| normalization       | ~CMN per chunk of final keep~ |
| steps_per_chunk     | 8 or whatever doesn't crash   |
|---------------------+-------------------------------|
| val_dp_shuffle_seed | None                          |
| val_dp_callids      | ~('00003', '00086')~          |
|---------------------+-------------------------------|
...
|---------------------+-------------------------------|
| nclasses            | 3                             |
| input_shape         | trn_dp.input_shape            |
| model               | ~c3~                          |
|---------------------+-------------------------------|
| trn_steps_per_epoch | trn_dp.nchunks                |
| val_steps           | val_dp.nchunks * 8            |
| max_q_size          | 32                            |
| verbosity           | 2                             |
| pickle_safe         | True                          |
| model_checkpoints   | per-epoch ~filename-pattern~  |
| confusions          | init + per-epoch on val_dp    |
| tensorboard         | per-epoch                     |
|---------------------+-------------------------------|
**** Variables, and order of experiments


*** TODO [0/0] Post Processing / Smoothing / Inference
*** TODO [0/0] Evaluations and Comparisons
...
*** TODO [0/2]  4:29 PM : More progress, but shit there's so much still left.
...
- [ ] Decide on that validation call(s) to be used while training, based on analysis.
