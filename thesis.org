...
towards smaller durations. Overlaps longer than $4\,\text{seconds}$ are less common and
are often characteristic of the scenario of the conversation (e.g. politically
charged debates) if not otherwise a result of erroneous transcription. For
various conversational datasets, the reported median value is typically around
$0.5\,\text{seconds}$, and the distribution has a very thin tail in longer durations
\cite{heldner_pauses_2010}. Knowing this, a system that aims to classify each
each frame for the number of active speakers in it will face additional
challenge due to the heavy imbalance between the classes.
...
least an ideal one, is nearly perfect for the task of automating annotation of
...
of (as of yet) any existing work in open literature that use DCNNs for the task of
remedying what has been termed the /Achille's heel/ of speaker diarization
systems.
...
- **Acoustic Features:** :: Inspired by recent studies, the goal to circumvent
     ...
     **64-dimensional $\text{log}_{10}\text{-Mel-Filterbank Coefficients}$** extracted every
...
- **Temporal Smoothing:** :: The raw predictions from the frame-wise classifier
     ...
     conversation. The impact and possible improvement in performance by using a
     simplified version of the Viterbi algorithm for temporally smoothing the
     raw predictions will also be analyzed (Section [[Temporal Smoothing]]).
* WAIT [6/13] Approach
...
video recording and/or a transcription available.
...
acoustic feature extraction; more details in Section [[Data Preparation and
Analysis]].)
...
achieve record-breaking performance on multiple occasions in recent years.
...
the appropriate classifier, whereas /unsupervised learning/ methods don't have
...
temporal localization, the proposed DCNN based classifier (with parameters \theta) predicts the
conditional probability $p(C_j | \textbf{x}_t, \theta)$ of each acoustic feature vector
$\textbf{x}_t$ extracted from the audio at time $t$ for having speech from the
$j^{th}$ class of $C =$ /**{0 speakers, 1 speaker, more than 1 speakers}**/. The
final decision $y_t$ can then be made by choosing the class that was assigned with
the maximum probability, as
$$y_t = \argmax_j p(C_j | \textbf{x}_t, \theta)\dot$$
...
A decoding scheme that can incorporate such statistics can potentially perform
much better than the naive one being used so far. In fact, it very common for
realistic frame-wise classifiers to produce relatively noisy predictions, and
the /raw/ sequence of labels $\textbf{y}^{(raw)}$ obtained using the naive decoding
method could end up containing such short contiguous durations of these events
which might be physically impossible (or at least highly unlikely).
It is therefore common to /smooth/ out such erroneous predictions using more sophisticated
decoding schemes to obtain a better sequence of labels $\textbf{y}^{(smooth)}$.

The most popular method used for modelling sequential data is with the use Hidden
Markov Models (HMMs).
...
[[Speaker Diarization]]), in the approaches that used a GMM-HMM framework for overlap
...
components. However, with the small datasets that was used by those works, such
...
problem during this thesis's work. The most important one was to choose the
...
Furthermore, the probability of picking a sample was specifically chosen so that
the final under-sampled dataset has a ratio of 2:1 between single-speaker and
...
their combinations (propotionally) equally. Given that the Fisher Corpus has a
maximum of two speakers in every conversation and that individual speakers can
be propotionally represented by the above procedure, the final ratios will end
up as 1:1:1 between the first speaker, second speaker, and their overlap,
respectively, for some choice of the skipping probability. This is, again, an
approximation, and future works should investigate different parameters, or
other rebalancing techniques \cite{huang_learning_2016} to achieve such goals if
they are worthwhile.

The implementation done to do this for this thesis' works performed such
...
supported by the desire to avoid any impact on convergence or of biased variance
for the under-sampled class, in experiments where such under-sampling was
performed, the neural network was trained for at least twice as many epochs than
in experiments where none of the classes were under-sampled. Lastly, in all
configurations where such undersampling was performed, silence frames were
skipped, to avoid the impact of such frames (Section [[Removing Silence]]).
...
misguiding. For example, a classifier that predict every sample to belong to the
single-speaker class will achieve an overall $81.20\%$ accuracy score (on the
testing set in Section [[Data Preparation and Analysis]]).
...
total duration of falsely detected overlap (aka False Alarms)
$T_{false}^{(ov)}$. The summary metric used for the reporting these errors, made
by a system that assigned a total duration of $T_{sys}^{(ov)}$ as overlaps in
...
\frac{T_{ref}^{(ov)} - T_{miss}^{(ov)}}{T_{ref}^{(ov)}}\ldot$$
...
\cite{morgan_meeting_2001}, etc. \cite{cetin_speaker_2006} have been used by
...
|--------------------------------------------+--------------|
#+LATEX: \caption{
Rough sizes of different conversational datasets used in
overlapping speech detection, and that of the Fisher Corpus - Part 1
#+latex:\emph{
(size of NIST RT is the one reported in \cite{zelenak_detection_2012}).
#+LATEX: }}
...
is $10\,\text{milli-seconds}$, which govern the frame-rate of the acoustic
...
*** DONE Data Preparation and Analysis
...
done on the testing set. For simplicity, these sets were made
in a sequential order as follows: the first 99 calls (\sim16.71 hours, 1.74%) were
assigned to the validation set, next 4,000 calls (\sim652.85 hours, 68.00%) to the
training set, and the last 1,751 calls (\sim290.74 hours, 30.26%) to the testing
set.
...
#+LATEX: \caption{Weighted histogram of durations of segments (truncated to $<4$ sec. long) with different number of active speakers in the final \textbf{training set}.}
#+LATEX: \label{fig:actspk-hist-trn}
#+LATEX: \end{figure}

#+LATEX: \begin{figure}
#+LATEX: \includegraphics[width=\textwidth]{img/actspk-whist-tst}
#+LATEX: \caption{Weighted histogram of durations of segments (truncated to $<4$ sec. long) with different number of active speakers in the final \textbf{testing set}.}
#+LATEX: \label{fig:actspk-hist-tst}
#+LATEX: \end{figure}

Table \ref{tab:actspk-sets} shows the ratios of the segments and the total
durations with different number of active speakers for the final sets.
Figure \ref{fig:actspk-hist-trn} shows the histogram of propotional (weighted by
duration) contributions of segments of different lengths to the total set of
segments with respective number of speakers active in the training set. Table
\ref{tab:actspk-tst} and Figure \ref{fig:actspk-hist-tst} do the same for the
final testing set. It can be seen that,
...
the data matche what has been reported for other conversational datsets used in
other overlapping speech detection systems.
# #+LATEX: \begin{table}
#   |-----------------+---+-----------------+---+---------------|
#   |-----------------+---+-----------------+---+---------------|
#   | /# of Speakers/ |   | /# of Segments/ |   | /# of Frames/ |
...
these $\text{log}_10\text{-Mel-Filterbank Coefficients}\ldot$ These were
...
To improve these features further, and as is common in many other works, each
feature vector, before being input to the proposed DCNN, were attached with neighboring
frames so as to provide contextual acoustic information. This was
motivated by the understanding that contextual information could help predict
occurrences of double-talk situations, as was discussed in Section [[Double-Talk in
Conversations]]. Furthermore, it had been reported in previous works that certain
long-term acoustic features like prosody estimates \cite{zelenak_speaker_2012},
or even statistics of silences and speaker changes \cite{yella_overlapping_2014}
helped improve the results over using acoustic features alone. Knowing that DCNNs
are capable of learning low- and high-level representations of data for an
appropriate task, it was hoped that the proposed classifier could learn such
longer-term patterns from the contextual acoustic information automatically. The
number of contextual frames added were ±10 (from before and after the center
frame) resulting in the total acoustic information available to the classifier
being $210\,\text{ms}$. The particular decision was based on initial empirical
evaluations and some practical concerns (Section [[Architecture]]).
*** WAIT Normalization
...
predicted as \sp was been avoided.
