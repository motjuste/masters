...
** DOIN [0/31] Finale Planne whatever
...
- [ ] look at links on MFCC, iVectors, prosody, pitch, CNSC, PLP, etc. for inspiration.
  + [ ] See if the argument that 'spectrogram' is the mother of all features
    still holds.
*** DOIN [0/13] The Classifier and Configurations
**** WAIT [0/2] The Neural Networks - {c3}
...
**** TODO [0/0] Skipping class(-es)
**** TODO [0/0] Class-/Sample-Weights
**** TODO [0/0] Fixed number of steps per chunk, {8, whatever runs, likely one with 2k examples}
- This is to make sure we can predict exactly how many steps are required for a pass
**** WAIT [0/4] Number of epochs / passes over the dataset - 8 epochs per pass * {1, 6, 11, 21 passes}
...
- [ ] upto 20 total passes for the best/most promising/relevant models.
  ...
  - [ ] pass it as a parameter to Keras.fit_generator?
- 8 keras epochs per pass anyways
- It is okay if we pass over a little more due to rounding, but we don't want to
  pass less than the entire dataset.
**** WAIT [0/1] Save model on every keras epoch {every-one}
- [ ] decide on file name formatting.
- Save checkpoints every epoch
**** WAIT [0/2] Save Tensorboard events
...
- [ ] possibly a give the same files as those given for the confusions calculations
...
**** WAIT [0/7] Confusions calculations in callback
...
**** IDEA [/] Predict over validation and/or test dataset at the end of training, in the script
...
*** TODO [0/1]  9:19 PM : Couldn't take a break, updated network
...
- [ ] Add the approach of filtering away /after/ sequences have been made, in
