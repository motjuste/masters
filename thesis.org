...
** [0/3] Objectives and expected challenges                           :cite:
The larger project that the work done in thesis is a part of, aims towards
improving the conversational analysis workflows performed in the area of linguistics. A diarization system, at least
an ideal one, is nearly perfect for the task of automating annotation of
conversations which are otherwise done manually and cost time and expertise.
Furthermore, as noted earlier, this system also assists the speech recognition
system in it's automatic transcription task. As discussed in Section [[Speaker
Diarization]], an overlap detection system can help improve the overall
performance of a diarization system, especially when applied to conversational
scenarios containing a lot of overlapping speech. And finally, as motivated in
Section [[Double Talk in Conversations]], a system that can detect double talk
situations itself is directly valuable to conversation analysis. The ultimate goal of the
work done in thesis, within this context, is the automatic detection and temporal
localization of double talk (aka overlapping speech) that occur in conversations.
...
detection system. As previously motivated, these technolgies can help avoid the
need for manually engineering problem-specific features (a common theme in the
existing approaches), while promising ground breaking and well generalizable
results. The task of detecting overlapping speech has proven to be extremely
challenging, especially in case of the short duration ones (that nonetheless
make the biggest contribution), so it is perhaps worthwhile to investigate the
powerful promises of deep learning methods. If successful, the deep neural
network could have learned the appropriate representations from low-level
acoustic features that best discriminate and predict the number of speakers
active at a particular time in an audio recording (0, 1, or 2 speakers).
Nevertheless, even before starting, many opportunities to tune the system exist,
from the nature of low-level acoustic features being used as inputs, to the
configurations of various layers in the deep neural network, how they are
trained and later fine-tuned, etc. These result in a combinatorial explosion of
methologies that should ideally be investigated in a comprehensive study, and is
especially warranted by the lack of (as of yet) any existing published work along the same
lines.

Restrictions imposed to the duration of this thesis, which are further
taxed by the amount of computation time and power necessary for properly working
with deep learning technologies, limit the number of possible approaches that
could be investigated in the allotted time. Therefore, in addition to prioritizing
more straightforward approaches in the investigations, an important goal of the work done for this
thesis was to also implement an end-to-end pipeline for speech segmentation
research (and perhaps deployment) based on reputed open source technologies
which can be used for further research with related goals. The former
comprise the rest of thesis starting in Chapter [[Methodology]], while the latter
are appropriately discussed as per need.

In the rest of this section, the objectives are formalized further alongside the
expected challenges that shape them.

- We still attempt to solve a more difficult version of the task.
  - mono-aural
  - Speaker identity and number of speaker and language indpendent model.
    - Assuming it to be a purely acoustic phenomena.
    - we still limit ourselves to 2-speaker overlaps

- Incredible recent results using deep learning.
- Cons:
  + Need lots of data.
  + Hence Fisher, even though other reason is more practical.
